...
**Before configuring a node, make sure you chose the RIGHT node.** Use `search_nodes` to find options, then apply these rules:

### AI/LLM Node Selection

```
Need: Simple LLM call (classify, summarize, extract, generate)
  → Use: OpenAI node (@n8n/n8n-nodes-langchain.openAi v2.1)
  → Config: resource="text", operation="response"
  → No sub-nodes needed. Direct API call.

Need: LLM with tools, memory, or multi-step reasoning
  → Use: AI Agent (@n8n/n8n-nodes-langchain.agent)
  → Requires sub-nodes: model + tools + optional memory

AVOID: Basic LLM Chain (@n8n/n8n-nodes-langchain.chainLlm)
  → Legacy LangChain wrapper. Unnecessarily complex.
  → Always prefer the OpenAI node for simple tasks.
```

### Form Node Selection

```
Need: Start workflow from a web form submission
  → Use: Form Trigger (n8n-nodes-base.formTrigger, typeVersion: 2.3)
  → This is the TRIGGER node that starts execution.

Need: Multi-page form (additional pages after trigger)
  → Use: n8n Form (n8n-nodes-base.form)
  → REQUIRES a Form Trigger upstream. Not a standalone trigger.

CRITICAL: form ≠ formTrigger. Don't swap them.
```

### Common Selection Mistakes

| Mistake | Correct Choice | Why |
|---------|---------------|-----|
| Using `chainLlm` for simple classification | `openAi` (resource: text) | No chain abstraction needed |
| Using `form` as a trigger | `formTrigger` | `form` is for multi-page continuation only |
| Using `lmChatOpenAi` sub-node + `chainLlm` | `openAi` node alone | Direct API call, no sub-nodes |
| Using AI Agent for simple text generation | `openAi` (resource: text) | Agent is overkill without tools |
